...
impl QueryRoot {
    ...
    async fn get_update_streak(
        &self,
        ctx: &Context<'_>,
    ) -> Result<Vec<StreakUpdate>, sqlx::Error> {
        let pool = ctx.data::<Arc<PgPool>>().expect("Pool not found in context");
    ...
    async fn get_attendance_summary(
        ...
        let daily_count = attendance_days
            .into_iter().map(|(date, count)| DailyCount{
                date, count
            })
            .collect();

        let member_attendance = member_attendance
            .into_iter().map(|(id, present_days)| MemberAttendance{
                id, present_days
            })
            .collect();
    ...
    pub async fn get_projects(
        ...
        let active_projects = sqlx::query_as::<_, ActiveProjects>(
